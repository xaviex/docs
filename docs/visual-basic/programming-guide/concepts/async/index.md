...
##  <a name="BKMK_APIAsyncMethods"></a> API Async Methods  
 ...
 The Windows Runtime also contains many methods that you can use with `Async` and `Await` in Windows apps. For more information and example methods, see [Quickstart: using the await operator for asynchronous programming](http://msdn.microsoft.com/en-us/library/windows/apps/hh452713.aspx), [Asynchronous programming (Windows Store apps)](http://msdn.microsoft.com/en-us/library/windows/apps/hh464924.aspx), and [WhenAny: Bridging between the .NET Framework and the Windows Runtime](https://msdn.microsoft.com/library/jj635140(v=vs.120).aspx).  
